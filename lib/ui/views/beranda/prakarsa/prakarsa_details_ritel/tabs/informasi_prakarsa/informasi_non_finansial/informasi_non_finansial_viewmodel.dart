...
class InformasiNonFinansialViewModel extends BaseViewModel {
  String? prakarsaId;
  String? name;
  ...
  InformasiNonFinansialViewModel({
    this.prakarsaId,
    this.name,
  ...
  Future insertAnswer(int page, int valueAnswer) async {
    if (valueAnswer != 0) {
      ...
      _showErrorDialog(
        'Pilih terlebih dahulu jawaban dari list yang tersedia!',
      );
    }
  }
  ...
  Future postNonFinansialData() async {
    ...
    final res = await runBusyFuture(
      _ritelPrakarsaAPI.postPrakarsaNonFinansial(summaryData, codeTable),
    );
    ...
    res.fold(
      ...
      (success) {
        _showSuccessDialogPut('Success Save Data');
        _isAfterSubmit = true;
        notifyListeners();
      },
    );
