...
class InformasiUsahaDebiturPariViewModel extends BaseViewModel {
  String? pipelineId;
  ...
  Future _prepopulateTextFields() async {
    ...
    jenisKomoditasController
      ..text = ritelInformasiUsaha?.jenisKomoditas ?? ''
      ..selection =
          TextSelection.collapsed(offset: jenisKomoditasController.text.length);
  ...
  void updateRadioBtn(bool val) async {
    ...
    if (_sameAddress) {
      ...
      if (ritelInformasiUsaha!.detail != null) {
        addressController
          ..text = ritelInformasiUsaha?.tagLocation?.name ?? ''
          ..selection =
              TextSelection.collapsed(offset: addressController.text.length);
  ...
  void updatePostalCode(String val) async {
    ...
    // rtController.clear();
    // rwController.clear();
    await getPostalCode(postalCodeController.text);
  ...
  Future<Map<String, dynamic>> _generateInfoUsahaMap() async {
    ...
    return {
      ...
      'businessName': namaUsahaController.text.isNotEmpty
          ...
          : '',
      'jenisKomoditas': jenisKomoditasController.text.trim(),
      'economy_sectorsId':
      ...
      'tagLocation': _sameAddress
          ...
          : ritelInformasiUsaha!.tagLocation != null
              ? {
                  'latLng': ritelInformasiUsaha!.tagLocation!.latLng!,
                  'name': ritelInformasiUsaha!.tagLocation!.name!,
                }
              : {
                  'latLng': latLng,
  ...
  Future _postData(
    PipelinePeroranganPariAPIMethod pipelinePeroranganPariAPIMethod,
    bool isContinue,
  ) async {
    final infoUsahaMap = await _generateInfoUsahaMap();
    final dataDiriAddressMap = await _generateDataDiriAddressMap();

    switch (pipelinePeroranganPariAPIMethod) {
      case PipelinePeroranganPariAPIMethod.addDataDiriPari:
        break;
      case PipelinePeroranganPariAPIMethod.updateDataDiriPari:
        break;
      case PipelinePeroranganPariAPIMethod.addUsahaPari:
        final res = await runBusyFuture(
          _ritelPipelinePeroranganAPI.addInformasiUsahaPari(infoUsahaMap),
        );
        ...
        break;
      case PipelinePeroranganPariAPIMethod.updateUsahaPari:
        final res = await runBusyFuture(
          _ritelPipelinePeroranganAPI.updateInformasiUsahaPari(infoUsahaMap),
        );
        ...
        break;
      case PipelinePeroranganPariAPIMethod.addLainnyaPari:
        break;
      case PipelinePeroranganPariAPIMethod.updateLainnyaPari:
        break;
    }
  }
  ...
  Future validateInputs(
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (response != null && response.confirmed) {
        _postData(pipelinePeroranganPariAPIMethod, isContinue);
      }
    } else {
