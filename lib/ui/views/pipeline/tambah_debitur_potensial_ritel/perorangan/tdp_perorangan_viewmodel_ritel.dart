...
class TDPPeroranganViewModel extends FutureViewModel {
  String? pipelineId;
  ...
  Future navigateToInformasiDataDiri() async {
    if (pipelineId == null) {
      ...
      final response = await runBusyFuture(
        _ritelPipelinePeroranganAPI.getInformasiDataDiri(pipelineId!, 1),
      );
  ...
  Future navigateToInformasiUsaha() async {
    if (pipelineId != null && !checkUsahaCompleted) {
      _navigationService.navigateTo(
      ...
      final response = await runBusyFuture(
        _ritelPipelinePeroranganAPI.getInformasiUsaha(pipelineId!, 1),
      );
  ...
  Future navigateToInformasiLainnya() async {
    if (pipelineId != null && !checkLainnyaCompleted) {
      _navigationService.navigateTo(
      ...
      final response = await runBusyFuture(
        _ritelPipelinePeroranganAPI.getInformasiLainnya(pipelineId!, 1),
      );
      ...
      if (fromPipelineDetailsView == true) {
        if (statusPipeline == 1) {
          final res = await runBusyFuture(
            _ritelPipelinePeroranganAPI.savePipeline(pipelineId!),
          );
        ...
        res.fold(
          ...
          (successMap) async {
            final resRunPrescreening = await runBusyFuture(
              _ritelScreeningAPI.runPrescreening(pipelineFlagId!),
            );
